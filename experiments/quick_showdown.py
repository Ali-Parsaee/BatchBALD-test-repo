"""
QUICK SHOWDOWN: Fast comparison of top methods only.
"""
...
def test_strategy(
    ...
    probe_depth=3,
    batch_size=30,
    n_runs=10,  # Increased for stability
    verbose=True
):
    ...
    for run in range(n_runs):
        ...
        # Train initial model (reduce epochs for speed)
        ...
        model_init.fit(X_train, artificial_time, artificial_event, epochs=20, batch_size=32, verbose=False)  # Reduced epochs
        ...
        model_updated.fit(X_train, updated_time, updated_event, epochs=20, batch_size=32, verbose=False)  # Reduced epochs
        ...
        if verbose:
            print(f"Δ={improvement:+.4f} (reveals={n_revealed})")
...
def main():
    print("\n" + "="*80)
    print("QUICK SHOWDOWN: Key Methods Only")
    print("="*80 + "\n")
    ...
    strategies = {
        ...
        # Top BatchBALD variants
        'Plain BatchBALD': SurvivalBatchBALD(probe_depth=probe_depth),
        'Observable Mass': ImprovedBatchBALD(probe_depth=probe_depth, variant='observable_mass'),
        'Optimal (Adaptive)': OptimalBatchBALD(probe_depth=probe_depth, strategy='adaptive_fusion'),
    ...
    # Print table
    print(f"{'Rank':<5} {'Strategy':<25} {'Mean Δ':<15} {'Wins':<10} {'Reveals'}")
    print("-" * 80)

    for rank, s in enumerate(stats, 1):
        marker = "🏆" if rank == 1 else "🥈" if rank == 2 else "🥉" if rank == 3 else "  "
        print(f"{marker} {rank:<3} {s['Strategy']:<25} {s['Mean Δ']:+.4f}±{s['Std Δ']:.4f}   {s['Wins']}/{s['Total']}      {s['Avg Reveals']:.1f}")

    # Statistical significance vs Entropy
    print("\n" + "="*80)
    ...
    if 'Entropy' in all_results:
        ...
        print(f"{'Strategy':<25} {'Mean Diff':<15} {'p-value':<12} {'Significant?'}")
        print("-" * 70)
        ...
        for s in stats:
            ...
            try:
                ...
                print(f"{s['Strategy']:<25} {mean_diff:+.4f}          {p_val:.4f}      {sig}")
            except:
                print(f"{s['Strategy']:<25} {'N/A':<15} {'N/A':<12} {'N/A'}")
