"""
Compare original vs optimized BatchBALD implementations with BUDGET=20.
...
def run_comparison(n_trials=5, initial_samples=200, budget=20, increment=60, prefilter_size=500):
    ...
    print("="*70)
    print("BATCHBALD COMPARISON WITH BUDGET=20")
    print("="*70)
    print()
    print("Methods:")
    ...
    if len(all_results['BatchBALD']) > 0:
        bb_orig = [r['final_cindex'] for r in all_results['BatchBALD']]

        for method_name in ['Variance', 'Entropy', 'Random', 'BatchBALD_optimal (λ₁=0.0)', 'BatchBALD_optimal (λ₁=0.3)']:
            if len(all_results[method_name]) > 0:
                other_scores = [r['final_cindex'] for r in all_results[method_name]]
                t_stat, p_val = stats.ttest_rel(bb_orig, other_scores)
                diff = np.mean(bb_orig) - np.mean(other_scores)
                sig = "✓" if p_val < 0.05 else ""
                print(f"BatchBALD vs {method_name}:")
                print(f"  Mean difference: {diff:+.4f}, t={t_stat:.3f}, p={p_val:.4f} {sig}")
...
if __name__ == "__main__":
    all_results, summary = run_comparison(
        n_trials=5,
        initial_samples=200,
        budget=20,
        increment=60,
