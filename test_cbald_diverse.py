"""
Testing CBALD-Diverse: C-BALD scoring with diversity filtering.

GOAL: Beat C-BALD by adding simple diversity to proven winner!

Strategy:
1. Use C-BALD's superior scoring (information value weighting)
2. Add greedy diversity filtering (avoid redundant samples)
3. Simpler than BatchBALD's joint entropy, faster than BatchBALD-C

Tests:
1. CBALD-Diverse (NEW - C-BALD + diversity) ⭐
2. C-BALD (Current champion)
3. BatchBALD (Original)
4. Variance

Settings: 2 trials for quick test, budget=20
"""
...
from Model_stuff.acquisition import (
    ...
    entropy_of_probs,
    variance_of_probs,
    cbald_censored_regression,  # C-BALD (current champion)
    cbald_diverse_acquire,  # NEW: CBALD-Diverse (hybrid!)
    random_knapsack
)
...
def run_comparison(n_trials=5, initial_samples=200, budget=20, increment=60, prefilter_size=500):
    """Run full comparison across multiple trials."""

    print("="*70)
    print("TESTING CBALD-Diverse: Can it beat C-BALD?")
    print("="*70)
    print()
    print("Strategy: Use C-BALD's proven scoring + add diversity!")
    print("  1. Compute C-BALD scores (information value weighting)")
    print("  2. Select top 3x candidates by C-BALD score")
    print("  3. Greedily pick diverse subset (avoid redundancy)")
    print()
    print("Methods:")
    print("  1. CBALD-Diverse (NEW - C-BALD + diversity) ⭐")
    print("  2. C-BALD (Current champion)")
    print("  3. BatchBALD (Original)")
    print("  4. Variance")
    print()
    print("Settings:")
    ...
    # Define acquisition functions (NOW WITH BatchBALD-C!)
    acquisition_functions = [
        (cbald_diverse_acquire, 'CBALD-Diverse'),      # NEW: C-BALD + diversity!
        (cbald_censored_regression, 'C-BALD'),          # Current champion
        (batchbald_acquire_budget, 'BatchBALD'),        # Original
...
if __name__ == "__main__":
    ...
    print("\nCBALD-Diverse test complete! Did it beat C-BALD?")
